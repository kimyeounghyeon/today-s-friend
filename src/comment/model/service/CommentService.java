...
public class CommentService {
	...
	public List<Comment> myReplylist(int startRnum, int endRnum, String id) {
		List<Comment> CommentVOList = null;
		Connection conn = getConnection();
		CommentVOList = new CommentDao().myReplylist(conn, startRnum, endRnum, id);
		close(conn);
		return CommentVOList;
	}
	...
	public int getMyReplyCount(String id) {
		int cnt = 0;
		Connection conn = getConnection();
		cnt = new CommentDao().getMyReplyCount(conn, id);
		close(conn);
		return cnt;
	}
}

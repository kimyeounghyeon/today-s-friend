...
%>
		<div class="tdiv">
			<p class="pabouttitle">음식친구 - <%=loc %></p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<select id="selloc">
		...
		<div id="tool">
			...
			<a id="introducea" class="bar-item button"
				href="javascript:void(window.open('https://www.youtube.com/channel/UCyn-K7rZLXjGl7VXGweIlcA', '_blank'))">백종원의 요리비책</a>
			<a id="introduceb" class="bar-item button"
				href="javascript:void(window.open('<%=request.getContextPath()%>/page/map/food.jsp', '_blank','width=500px, height=400px'))">치킨지도</a> <a href="<%=request.getContextPath()%>/page/board/adread"
		...
		<div id="pagen">
			<c:if test="${startPage != 1 }">
				<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${startPage-1}&search=${search }&hobbyId=4&locnum=<%=locnum%>">이전</a>
			</c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="s" step="1">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${s }&search=${search }&hobbyId=4&locnum=<%=locnum%>">${s }</a>
			</c:forEach>
			<c:if test="${endPage < pageCnt }">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${endPage+1}&search=${search }&hobbyId=4&locnum=<%=locnum%>">다음</a>
			</c:if>
		</div>
	</article>
</section>

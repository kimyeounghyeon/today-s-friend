...
<section class="setop">
	<article>
		...
		<a href="<%=request.getContextPath()%>/page/board/boardwrite.jsp?hobbyId=6" id="write" class="write">글쓰기</a>
				...
				<table border="1">
					...
					<tr>
						<td colspan="2">
							...
							<button type="button" onclick="location.href='<%=request.getContextPath()%>/page/board/delete1?bno=${v.bno}&hobbyId=6'">삭제</button>
						</td>
					</tr>
					<form id="frmmod" class="frmmod">
						...
						<input type="hidden" name="hobbyId" value="6">
					</form>
				</table>
				<br>
			</c:forEach>
		...
		<c:if test="${startPage != 1 }">
			<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${startPage-1}&search=${search }&hobbyId=6">&#60;</a>
		</c:if>
		<c:forEach begin="${startPage }" end="${endPage }" var="s" step="1">
			<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${s }&search=${search }&hobbyId=6">${s }</a>
		</c:forEach>
		<c:if test="${endPage < pageCnt }">
			<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${endPage+1}&search=${search }&hobbyId=6">&#62;</a>
		</c:if>
	</article>
</section>

...
<div class="click"
	...
	<a
		href="<%=request.getContextPath()%>/page/board/boardwrite.jsp?hobbyId=5"
...
%>
		<div class="tdiv">
			<p class="pabouttitle">주식친구 - <%=loc %></p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<select id="selloc">
		...
		<div id="tool">
			...
			<a id="introducea" class="bar-item button"
				href="javascript:void(window.open('https://newsstand.naver.com/?list=ct1', '_blank'))">네이버 뉴스</a>
			<a id="introduceb" class="bar-item button"
				href="javascript:void(window.open('https://upbit.com/exchange?code=CRIX.UPBIT.KRW-BTC', '_blank'))">비트코인</a>
		...
		<c:if test="${not empty blist }">
			<form id="frmSearch">
				<input type="text" name="search" id="search"> <input
					type="hidden" name="hobbyId" value="5"> <input
				...
				<table border="1" class="trd">
					...
					<tr class="nline" id="vid">
						...
						<td class="btnn" style="text-align: right">
							...
							<button type="button" class="btn" id="ed"
								onclick="location.href='<%=request.getContextPath()%>/page/board/delete1?bno=${v.bno}&hobbyId=5'">삭제</button>
						</td>
					</tr>
		...
		<div id="pagen">
			<c:if test="${startPage != 1 }">
				<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${startPage-1}&search=${search }&hobbyId=5&locnum=<%=locnum%>">이전</a>
			</c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="s" step="1">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${s }&search=${search }&hobbyId=5&locnum=<%=locnum%>">${s }</a>
			</c:forEach>
			<c:if test="${endPage < pageCnt }">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${endPage+1}&search=${search }&hobbyId=5&locnum=<%=locnum%>">다음</a>
			</c:if>
		</div>
	</article>
</section>

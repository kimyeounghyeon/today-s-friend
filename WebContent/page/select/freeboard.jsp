...
<div class="click"
	...
	<a
		href="<%=request.getContextPath()%>/page/board/boardwrite.jsp?hobbyId=6"
...
%>
		...
		<div id="pagen">
			<c:if test="${startPage != 1 }">
				<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${startPage-1}&search=${search }&hobbyId=6&locnum=<%=locnum%>">이전</a>
			</c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="s" step="1">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${s }&search=${search }&hobbyId=6&locnum=<%=locnum%>">${s }</a>
			</c:forEach>
			<c:if test="${endPage < pageCnt }">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${endPage+1}&search=${search }&hobbyId=6&locnum=<%=locnum%>">다음</a>
			</c:if>
		</div>
	</article>
</section>

...
<div class="click"
	...
	<a
		href="<%=request.getContextPath()%>/page/board/boardwrite.jsp?hobbyId=2"
...
%>
		<div class="tdiv">
			<p class="pabouttitle">영화친구 - <%=loc %></p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<select id="selloc">
		...
		<c:if test="${not empty blist }">
			<form id="frmSearch">
				<input type="text" name="search" id="search" placeholder="회원아이디로 검색"> <input
					type="hidden" name="hobbyId" value="2"> <input
				...
				<table border="1" class="trd">
					...
					<tr class="nline" id="vid">
						...
						<td class="btnn" style="text-align: right">
							...
							<button type="button" class="btn" id="ed"
								onclick="location.href='<%=request.getContextPath()%>/page/board/delete1?bno=${v.bno}&hobbyId=2'">삭제</button>
						</td>
					</tr>
					...
					<tr class="nline" id="pan" bgcolor="#57d2b4">
						...
						<form id="frmmod" class="frmmod">
							...
							<input type="hidden" name="hobbyId" value="2">
						</form>
				</table>
				<br>
			</c:forEach>
		...
		<div id="pagen">
			<c:if test="${startPage != 1 }">
				<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${startPage-1}&search=${search }&hobbyId=2&locnum=<%=locnum%>">이전</a>
			</c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="s" step="1">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${s }&search=${search }&hobbyId=2&locnum=<%=locnum%>">${s }</a>
			</c:forEach>
			<c:if test="${endPage < pageCnt }">
				<a
					href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${endPage+1}&search=${search }&hobbyId=2&locnum=<%=locnum%>">다음</a>
			</c:if>
		</div>
	</article>
</section>

...
<section class="setop">
	<article>
		<p class="pabouttitle">음식친구</p>
		<a href="<%=request.getContextPath()%>/page/board/boardwrite.jsp?hobbyId=4" id="write" class="write">글쓰기</a>
				...
				<table border="1">
					...
					<tr>
						<td colspan="2">
							...
							<button type="button" onclick="location.href='<%=request.getContextPath()%>/page/board/delete1?bno=${v.bno}&hobbyId=4'">삭제</button>
						</td>
					</tr>
		...
		<c:if test="${startPage != 1 }">
			<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${startPage-1}&search=${search }&hobbyId=4">&#60;</a>
		</c:if>
		<c:forEach begin="${startPage }" end="${endPage }" var="s" step="1">
			<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${s }&search=${search }&hobbyId=4">${s }</a>
		</c:forEach>
		<c:if test="${endPage < pageCnt }">
			<a href="<%=request.getContextPath() %>/page/board/boardread?pageNum=${endPage+1}&search=${search }&hobbyId=4">&#62;</a>
		</c:if>
	</article>
</section>
